...
# Note: These patterns match complete lines only (for safety)
BOILERPLATE_PATTERNS = [
    ...
    r"^Zur\sStartseite\s*$",
    r"^Zum\sSeitenanfang\s*$",
    r"^Zum\sInhaltsverzeichnis\s*$",
    r"^zurück\s(zur)?\sübersicht\s*$",
    r"^Weiter\szu.*$",
    r"^Top\sNavigation\s*$",
    r"^Footernavigation\s*$",
    r"^Mobile\sVersion\s*$",
    r"^Seitenumbruch\s*$",
    r"^Seitenende\s*$",
    r"^Seiteninhalt\s*$",
    r"^Sprache\swechseln\s*$",
    
    # ============ NEWSLETTER & SUBSCRIPTION ============
    r"^Newsletter\sabonnieren\s*$",
    r"^Zum\sNewsletter\sanmelden\s*$",
    r"^Newsletter\sbestellen\s*$",
    ...
    r"^Seite\s?drucken\s*$",
    r"^Teilen\s*$",
    r"^Dokument\serstellen\s*$",
    r"^Link\skopieren\s*$",
    r"^Zur\sVorlesung\sanmelden\s*$",
    r"^Jetzt\sregistrieren\s*$",
    r"^Anmeldung.*erforderlich.*$",
    r"^Bitte\sklicken\sSie\shier.*",
    r"^lesen\sSie\smehr\s*$",
    r"^Diesen\sBeitrag\steilen\s*$",
    r"^Inhalt\sverbergen\s*$",
    
    # ============ METADATA & MISC ============
    r"^Info-Portal\s*$",
    r"^Programmwebseite\s*$",
    r"^Sitemap\s*$",
    r"^Notfall\s?nummern?\s*$",
    r"^Datum:\s*\d{1,2}\.\d{1,2}\.\d{2,4}",
    r"^In\sKürze\s*$",
    r"Mehr\sInformation(en)?",
    r"^Sharing\sis\scaring\s*$",
